...
AO_TRACING_DECISIONS_XTRACE_NOT_SAMPLED = 2
AO_TRACING_DECISIONS_NO_CONFIG = 3
AO_TRACING_DECISIONS_REPORTER_NOT_READY = 4
AO_TRACING_DECISIONS_NO_VALID_SETTINGS = 5
AO_TRACING_DECISIONS_TRACING_DISABLED = 6
  ...
  class TransactionSettings
    ...
    def initialize(url = nil, xtrace = '')
     ...
     if AppOpticsAPM.tracing_disabled? && !tracing_enabled?(url) ||
      ...
      metrics, sample, @rate, @source, _result = AppOpticsAPM::Context.getDecisions(*args)

      # if error != AO_TRACING_DECISIONS_OK
      #   AppOpticsAPM.logger.debug "[appoptics-apm/sample] Error getting the sampling decisions, error ##{error} "
      # end
